...
Every service exposes Swagger, and the gateway is the address the SPA actually uses:

```bash
curl -X POST http://localhost:5000/api/polls   -H "Content-Type: application/json"   -d '{"question":"Tabs or spaces?","options":["Tabs","Spaces"]}'
```

Worth testing beyond the happy path: voting twice returns `409`, editing after voting has started
returns `409`, closing without the creator token returns `403`, and voting after the poll closes
returns `409`.
...
ServiceEndpoints__ApiGatewayBaseUrl=https://pollbuilder-gateway-rj0d.onrender.com
...
```
VITE_API_BASE_URL=https://pollbuilder-gateway-rj0d.onrender.com
```
